...
const getDogBreed = async () => {
  ...
  const data = await response.json();
  return Object.keys(data.message); // Object.keys returns an array of keys from the object
};
...
export default function Page() {
  ...
  // [] means the effect will only run once, after the first render.

  // loadRandomDog();
  // this will call loadRandomDog every time the page renders. so, the image keeps changing even if we are not reloading the page.
